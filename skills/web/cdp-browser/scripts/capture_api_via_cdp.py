"""
Self-run capture of anti-bot collector / API traffic via CDP, against a single
locked SDK build.

Generic, target-agnostic reverse-engineering tool. Per-batch isolation uses
Target.createBrowserContext (a real incognito-like profile partition), so we
get a fresh session each time without the suspicious
...
def launch_chrome(profile: Path):
    if cdp_up():
...
async def capture_one_batch(batch_id: int, batch_dir: Path, sdk_target_path: Path, cfg) -> dict:
    ...
    async with CDPClient(browser_ws) as browser:
        ...
        async with CDPClient(tab_ws) as cdp:
            ...
            for rid, r in seen.items():
                url = r.get("url") or ""
                if any(d in url for d in cfg.sdk_domain) and "main.min.js" in url:
                    sdk_req = (rid, r)
                elif any(c in url for c in cfg.collector_path) and r.get("method") == "POST":
                    r["request_id"] = rid
                    posts.append(r)
...
def parse_args(argv):
    p = argparse.ArgumentParser(description="Capture anti-bot collector / API traffic via CDP.")
    ...
    p.add_argument("--domain", default=None, help="site domain (default: derived from --url)")
...
async def main(cfg):
    launch_chrome(cfg.profile)

    N = cfg.batches
    sdk_path = cfg.sdk_dir / "main.min.js"

    summary = []
    for i in range(1, N + 1):
        bd = cfg.samples / str(i)
        shutil.rmtree(bd, ignore_errors=True)
        meta = await capture_one_batch(i, bd, sdk_path, cfg)
        summary.append(meta)
        if i < N:
    ...
    if sdk_path.exists():
        ...
        (cfg.sdk_dir / "INFO.json").write_text(json.dumps({
            "sha256": h,
            "size": sdk_path.stat().st_size,
            "sdk_domain": cfg.sdk_domain,
            "captured_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
