# Stage 1: Encryption Chain Reverse-Engineering (Generic, One-Time)
...
Weights: byte=0 -> 45, byte=255 -> 6, others -> 1 (common to all versions)
...
### Full Cookie T Decryption Flow (for hybrid verification)
...
- Use the raw 16 bytes of `keys[17]`/`keys[16]` directly as the AES key; no wrapping needed
- nonce = `[r2mkaTime(4B), currentTime(4B)]`
- Ciphertext length encoding: <128 uses 1 byte, >=128 uses 2 bytes `[0x80|hi, lo]`
- When downloading mainjs over HTTP, you must concatenate with Buffer + `toString('utf-8')`; do not use string concatenation (it corrupts multi-byte characters)
