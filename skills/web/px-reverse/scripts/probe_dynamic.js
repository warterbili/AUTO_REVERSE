// For each DYNAMIC field, locate its assignment context in the SDK
//
// Usage: node probe_dynamic.js <diff_result.json> <hQ_map.json> <SDK path>
const fs = require('fs');
const path = require('path');

if (process.argv.length < 5) {
    console.error('Usage: node probe_dynamic.js <diff_result.json> <hQ_map.json> <SDK path>');
    process.exit(1);
}
...
function lineOf(idx) {
    // Convert char index → line number
    let line = 1, c = 0;
...
function findHQ(N) {
    // Match hQ(N) or hQ(<spaces>N<spaces>)
